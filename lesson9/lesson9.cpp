// lesson9.cpp : 使用uniform buffer数据
//
...
CM::VulkanMemory g_VertexMemory;
CM::VulkanBuffer g_VertexBuffer;

CM::VulkanMemory g_UniformMemory;
CM::VulkanBuffer g_UniformBuffer;
...
void CreateVertexBuffer()
{
	...
	g_VertexMemory = g_Device.AllocAndBindMemory(buffers, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
void CreateUniformBuffer()
{
	...
	g_UniformBuffer = g_Device.CreateBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_SHARING_MODE_EXCLUSIVE, sizeof(projectionMatrix));
	std::vector<CM::VulkanBuffer*> buffers =
	{
		&g_UniformBuffer,
	};
	g_UniformMemory = g_Device.AllocAndBindMemory(buffers, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

	void* stagingStart = g_StagingMemory.Map(g_StagingBuffer.getOffset(), g_StagingBuffer.getSize());
	memcpy(stagingStart, projectionMatrix, sizeof(projectionMatrix));
	g_StagingMemory.Flush(g_StagingBuffer.getOffset(), sizeof(projectionMatrix));
	g_StagingMemory.Unmap();
	...
	{
		{
			0,                   // VkDeviceSize                           srcOffset
			0,                   // VkDeviceSize                           dstOffset
			sizeof(projectionMatrix) // VkDeviceSize                           size
		}
	};
	cb.CopyBuffer(g_StagingBuffer, g_UniformBuffer, buffer_copy_infos);

	VkBufferMemoryBarrier buffer_memory_barrier = g_UniformBuffer.CreateBufferMemoryBarrier(VK_ACCESS_MEMORY_WRITE_BIT, VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT);
	cb.PipelineBarrier(VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT, 0, 0, nullptr, 1, &buffer_memory_barrier, 0, nullptr);
	cb.EndCommandBuffer();
...
void CreateDescriptorSet()
{
	...
	std::vector<CM::VulkanDescriptorSet::BufferInfo> buffers =
	{
		{ &g_UniformBuffer, 0, g_UniformBuffer.getSize() },
	};

	g_DescriptorSets[0].Update(1, buffers);
}
