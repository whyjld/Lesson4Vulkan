...
void CreateVertexBuffer()
{
	...
	g_VertexBuffer = g_Device.CreateBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_SHARING_MODE_EXCLUSIVE, sizeof(vertexData));
	std::vector<CM::VulkanBuffer*> buffers =
	{
		&g_VertexBuffer,
	};
	g_BufferMemory = g_Device.AllocAndBindMemory(buffers, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
	void* ptr = g_BufferMemory.Map(g_VertexBuffer.getOffset(), g_VertexBuffer.getSize());
	memcpy(ptr, vertexData, sizeof(vertexData));
	g_BufferMemory.Flush(g_VertexBuffer.getOffset(), g_VertexBuffer.getSize());
	g_BufferMemory.Unmap();
...
bool RecordCommandBuffers(RenderResource& rr, uint32_t image)
{
	...
	rr.CommandBuffer.BeginCommandBuffer(VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT);
